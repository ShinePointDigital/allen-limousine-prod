...
import {
  ...
  Pencil, Plane, Plus, Search, Settings, ShieldCheck, Sparkles, UserRound, Trash2, WalletCards, X,
} from "lucide-react";
import "./dispatch.css";
...
type Inquiry = { id: string; fullName: string; email: string; phone: string; serviceType: string; pickupAt: string; pickup: string; destination: string; passengers: number; notes?: string; airportCode?: string | null; airportTerminal?: string | null; flightNumber?: string | null; flightScheduledAt?: string | null; pickupPreference?: string | null; isPrivateFBO?: boolean; specificTailNumber?: string | null; principalName?: string | null; fboName?: string | null; tarmacInstructions?: string | null; status: string; createdAt: string; updatedAt: string; history: { body: string; author: string; createdAt: string }[] };
...
type Ride = { id: string; inquiryId: string; status: string; driverName: string | null; driverPhone: string | null; vehicleId: string | null; quoteCents: number; depositCents: number; collectedCents: number; expenseCents: number; dispatchNotes: string | null; createdAt: string; updatedAt: string; inquiry: Pick<Inquiry, "fullName" | "serviceType" | "pickupAt" | "pickup" | "destination" | "passengers" | "notes" | "isPrivateFBO" | "specificTailNumber" | "principalName" | "fboName" | "tarmacInstructions">; vehicle: Pick<Vehicle, "id" | "name" | "category" | "active"> | null; dispatchMessages: DispatchActivity[] };
...
const BOOKING_DESTINATIONS = [
  ...
  { code: "FBO", title: "Private Aviation / FBO", image: "/images/private-fbo-night.jpg", destination: "", isPrivateFBO: true },
];

function DestinationBookingCards() {
  const bookDestination = (destination: string, isPrivateFBO = false) => {
    window.dispatchEvent(new CustomEvent("allen-booking-prefill", { detail: { destination, isPrivateFBO } }));
  ...
  return <section className="destination-booking-section" aria-labelledby="destination-booking-title">
    ...
    <div className="destination-booking-grid">
      {BOOKING_DESTINATIONS.map(item => <article className="destination-booking-card" key={item.code}>
        ...
        <button type="button" onClick={() => bookDestination(item.destination, item.isPrivateFBO)}>Book this route <ArrowUpRight /></button>
      </article>)}
    </div>
  </section>;
}
